...
* [`src/cljs/striker/`](src/cljs/striker/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/striker/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/striker/`](test/cljs/striker/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `striker` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/striker/config.cljs) defaults to `true` in
...
```clj
(ns striker.example
  (:require [striker.config :as config])
